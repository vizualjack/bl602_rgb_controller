...
void update_channel_duty(int channel_index, float duty) {
    if(channel_index <= -1 || channel_index >= 4) return;
    if(duty < 0.f || duty > 100.f) return;
    current_channel = channel_index;
    current_channel_duty = duty;
    need_to_update_channel_duty = true;
}

void set_channel_duty(int channel_index, float duty) {
    if(channel_index <= -1 || channel_index >= 4) return;
    if(duty < 0.f || duty > 100.f) return;
    bl_pwm_set_duty(channel_pin_map[channel_index][0], duty);
}

void set_color_duty(int color_to_channel_index, float duty) {
    int index = color_to_channel_pin_map[color_to_channel_index];
    set_channel_duty(index, duty);
}

void set_red_duty(float duty) { set_color_duty(0, duty); }
void set_green_duty(float duty) { set_color_duty(1, duty); }
void set_blue_duty(float duty) { set_color_duty(2, duty); }
void set_white_duty(float duty) { set_color_duty(3, duty); }
void set_rgb_duty(float red_duty, float green_duty, float blue_duty) {
    set_red_duty(red_duty);
    set_green_duty(green_duty);
    set_blue_duty(blue_duty);
}
void set_rgbw_duty(float red_duty, float green_duty, float blue_duty, float white_duty) {
